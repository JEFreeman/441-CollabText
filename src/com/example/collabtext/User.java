...
public class User {
		...
		collectText = new TextWatcher(){
		     ...
		     public void beforeTextChanged(CharSequence s, int start, int count, int after) {
				...
				if(beforeStart != start){
					if(count > after){
							...
							docEdit.broadcastMessage(REMOVE, move_to_add.start, move_to_add.change);
							checkUpdate();
							beforeStart = start;
						}
				}
			...
			@Override
		     public void onTextChanged(CharSequence s, int start, int before, int count) {		
					if(prevStart != start){
						if(count > before){
							Log.w("PrevStart:", String.valueOf(prevStart));
								...
								printMove(move_to_add);
								undoRedoList.add(move_to_add);
								docEdit.broadcastMessage(INSERT, move_to_add.start, move_to_add.change);
								checkUpdate();
								prevStart = start;
						}
					}				
					...
					Log.i("cur_move.change", s.toString().substring(start, start+count));
	...
	void checkUpdate(){
		...
		while(!globalChange.isEmpty()){
			...
			if(tempGlobal.type){
				globalText.delete(tempGlobal.start, tempGlobal.start + tempGlobal.length);
			}
			else{
